...
actor APIClient {
    ...
    func request<T: Decodable>(target: ServiceTargetProtocol) async throws -> (T, URLResponse) {
        ...
        debugResponse(request: urlRequest, data: data, response: response)
...
extension APIClient {
    ...
    func debugResponse(request: URLRequest, data: Data? = nil, response: URLResponse? = nil, error: Error? = nil) {
#if DEBUG
        Swift.print("============================ REQUEST ============================")
